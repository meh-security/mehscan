...
| XPath queries (CWE-643) | C#, Java | Typed `XPathNavigator`/`XmlNode` and `javax.xml.xpath.XPath` expression consumers | XPath grammar is captured separately from the XML document and evaluator arguments. Same-name receiver lookalikes are excluded. |
| Native process launch (CWE-78) | C, C++ | `execl*`, `execvp*`, `posix_spawn*`, `CreateProcess*`, `ShellExecute*` and `WinExec` | Executable and argument operands remain separate so review can distinguish shell text, executable selection and argument injection. |
...
| Process execution (CWE-78) | Standard process and shell launch APIs cover managed runtimes plus common POSIX and Windows native launch families, with command/argument separation represented independently. | Framework wrappers require proof that they reach a process boundary. Generic callback or method invocation is not a process sink. |
...
| HTML output (CWE-79) | Response writers, browser DOM assignment, Angular trust bypasses, React dangerous HTML, imported Lit `unsafeHTML`, imported Vue render-function `innerHTML`, Solid JSX `innerHTML`, and server-framework trusted-markup APIs cover the strong sink candidates. | Angular `[innerHTML]` remains framework-sanitized unless a trust bypass is used. Vue and Svelte single-file template directives need component-file parsing before `v-html` or `{@html}` can be modeled without text matching; broad `render`, `write` and `html` names remain too ambiguous. |
| XPath injection (CWE-643) | Typed Java and .NET XPath consumers preserve the expression operand and participate in bounded request-to-expression relationships. Dynamic expressions retain a decision-critical origin question. | PHP DOMXPath, Python lxml and package-owned XPath engines in other languages remain candidates. |
