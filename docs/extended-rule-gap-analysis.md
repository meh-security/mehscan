...
| HTML output (CWE-79) | Response writers, trusted-markup bypasses, browser DOM assignment, React dangerous HTML and framework-specific output contexts cover the strong sink candidates. | Additional UI-framework sinks need exact property/context modeling; broad `render`, `write` and `html` names are too ambiguous. |
