...
This analysis compares Mehscan's 419-rule embedded catalog and procedural
...
| XPath queries (CWE-643) | C#, Java, PHP, Python | Typed .NET/JVM consumers, native `DOMXPath` receivers, and lxml constructors or locally constructed parsed trees | XPath grammar is captured separately from the XML document and evaluator arguments. Same-name receiver lookalikes are excluded. |
| Native and runtime process launch (CWE-78) | C, C++, Go, Java, JavaScript, TypeScript, TSX, Python, PHP, Rust | Added native wide/privileged launch variants, `os.StartProcess`, `syscall.Exec`, `ProcessBuilder.command`, Node `fork`, Python async/exec/spawn siblings, PHP backticks and Tokio `Command` | Executable and argument operands remain separate so review can distinguish shell text, executable selection and argument injection. |
| Request and trusted-markup mutation | C#, Java, JavaScript, TypeScript, TSX, Python, Go, Rust | `HttpRequestMessage.RequestUri`, JDK `HttpRequest.Builder.uri`, Axios configuration-object URLs and method siblings, Requests/HTTPX/Reqwest method siblings, Go `http.Head`, and Go trusted markup constructors beyond `template.HTML` | These are security-sensitive property/builder mutations, exact request APIs, or explicit trust conversions with stable framework identity and a distinct operand. |
...
| --- | --- | --- |
| SQL and NoSQL injection (CWE-89, CWE-943) | The extended database pass covers the strong driver and document-query candidates for all 12 languages, including native asynchronous libpq/MySQL entrypoints and SQLite UTF-16 preparation. C# also carries exact controller parameters through one service and one repository call when every target is unique. | Wrapper factories, compiler-resolved overloads, inferred SDK fields and deeper or transformed cross-file flows remain bounded gaps. See [extended database coverage](extended-database-coverage.md). |
| Process execution (CWE-78) | Standard process and shell launch APIs cover managed runtimes plus common POSIX and Windows native launch families. Builder/property mutations and executable, argument and environment operands are represented independently where the API exposes them. | Framework wrappers require proof that they reach a process boundary. Generic callback or method invocation is not a process sink. |
| Outbound requests (CWE-918) | Declarative rules plus typed Java, Kotlin, C# and Python summaries cover the principal HTTP clients, URL operands and common request builder/property mutations. Axios configuration-object URLs are also preserved. | Add Playwright/Puppeteer navigation only after receiver construction and lazy-versus-dispatched request identity can be preserved. |
...
| HTML output (CWE-79) | Response writers, browser DOM assignment, Angular trust bypasses, React dangerous HTML, imported Lit `unsafeHTML`, imported Vue render-function `innerHTML`, Solid JSX `innerHTML`, server-framework trusted-markup APIs, and Go `html/template` trusted-type conversions cover the strong sink candidates. | Angular `[innerHTML]` remains framework-sanitized unless a trust bypass is used. Vue and Svelte single-file template directives need component-file parsing before `v-html` or `{@html}` can be modeled without text matching; broad `render`, `write` and `html` names remain too ambiguous. |
| XPath injection (CWE-643) | Typed Java and .NET XPath consumers, PHP `DOMXPath`, and Python lxml preserve the expression operand and participate in decision-critical review. Receiver validation uses exact types, constructors or a unique local construction; it does not infer application data flow. | Package-owned XPath engines in other languages remain candidates when receiver ownership can be established locally. |
...
- Header and SMTP injection (CWE-93/113), template injection
