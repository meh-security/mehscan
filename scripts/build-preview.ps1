...
$releaseName = "mehscan-v$Version-windows-$architecture"
...
try {
    ...
    $binaryPath = Join-Path $targetRoot 'release\mehscan.exe'
    $requiredSources = @(
        $binaryPath,
    ...
    if (-not $SkipSmoke) {
        ...
        $actualFiles = @(Get-ChildItem -LiteralPath $smokeRoot -File -Recurse | ForEach-Object {
            $_.FullName.Substring($smokeRoot.Length).TrimStart('\').Replace('\', '/')
        } | Sort-Object)
        ...
        $extractedBinary = Join-Path $smokeRoot 'mehscan.exe'
